...
public final class ConcurrentLinkedListKt {
    ...
    public static final <S extends Segment<S>> Object findSegmentInternal(S s, long j, Function2<? super Long, ? super S, ? extends S> function2) {
        while (true) {
            if (s.id >= j && !s.isRemoved()) {
                return SegmentOrClosed.m3003constructorimpl(s);
            }
            Object nextOrClosed = s.getNextOrClosed();
            if (nextOrClosed == CLOSED) {
                return SegmentOrClosed.m3003constructorimpl(CLOSED);
            }
    ...
    public static final /* synthetic */ <S extends Segment<S>> boolean moveForward$atomicfu(Object obj, AtomicReferenceFieldUpdater atomicReferenceFieldUpdater, S s) {
        while (true) {
            ...
            if (C0129xc40028dd.m88m(atomicReferenceFieldUpdater, obj, segment, s)) {
    ...
    public static final /* synthetic */ <S extends Segment<S>> boolean moveForward$atomicfu$array(AtomicReferenceArray atomicReferenceArray, int i, S s) {
        while (true) {
            ...
            if (C1443xa7a47114.m870m(atomicReferenceArray, i, segment, s)) {
                if (segment.decPointers$kotlinx_coroutines_core()) {
                    segment.remove();
                }
                return true;
        ...
        loop0: while (true) {
            ...
            if (!SegmentOrClosed.m3008isClosedimpl(findSegmentInternal)) {
                ...
                while (true) {
                    ...
                    if (C0129xc40028dd.m88m(atomicReferenceFieldUpdater, obj, segment, m3006getSegmentimpl)) {
                        if (segment.decPointers$kotlinx_coroutines_core()) {
                            segment.remove();
                        }
        ...
        loop0: while (true) {
            ...
            if (!SegmentOrClosed.m3008isClosedimpl(findSegmentInternal)) {
                ...
                while (true) {
                    ...
                    if (C1443xa7a47114.m870m(atomicReferenceArray, i, segment, m3006getSegmentimpl)) {
                        if (segment.decPointers$kotlinx_coroutines_core()) {
                            segment.remove();
                        }
                    } else if (m3006getSegmentimpl.decPointers$kotlinx_coroutines_core()) {
