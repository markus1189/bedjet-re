...
public class CrashlyticsController {
    static final FilenameFilter APP_EXCEPTION_MARKER_FILTER = new FilenameFilter() { // from class: com.google.firebase.crashlytics.internal.common.CrashlyticsController$$ExternalSyntheticLambda2
    ...
    synchronized void handleUncaughtException(final SettingsProvider settingsProvider, final Thread thread, final Throwable th, final boolean z) {
        Logger.getLogger().m891d("Handling uncaught exception \"" + th + "\" from thread " + thread.getName());
        ...
        Task submitTask = this.crashlyticsWorkers.common.submitTask(new Callable<Task<Void>>() { // from class: com.google.firebase.crashlytics.internal.common.CrashlyticsController.2
            ...
            public Task<Void> call() throws Exception {
                ...
                if (currentSessionId != null) {
                    ...
                    return settingsProvider.getSettingsAsync().onSuccessTask(CrashlyticsController.this.crashlyticsWorkers.common, new SuccessContinuation<Settings, Void>() { // from class: com.google.firebase.crashlytics.internal.common.CrashlyticsController.2.1
                        ...
                        public Task<Void> then(Settings settings) throws Exception {
                            if (settings == null) {
                                Logger.getLogger().m899w("Received null app settings, cannot send reports at crash time.");
                                return Tasks.forResult(null);
                            }
                ...
                Logger.getLogger().m893e("Tried to write a fatal exception while no session was open.");
                return Tasks.forResult(null);
            }
        });
        if (!z) {
            try {
                ...
                Logger.getLogger().m893e("Cannot send reports. Timed out while fetching settings.");
            } catch (Exception e) {
                Logger.getLogger().m894e("Error handling uncaught exception", e);
            }
        }
    }
    ...
    private Task<Boolean> waitForReportAction() {
        if (this.dataCollectionArbiter.isAutomaticDataCollectionEnabled()) {
            Logger.getLogger().m891d("Automatic data collection is enabled. Allowing upload.");
            this.unsentReportsAvailable.trySetResult(false);
            return Tasks.forResult(true);
        }
        Logger.getLogger().m891d("Automatic data collection is disabled.");
        Logger.getLogger().m897v("Notifying that unsent reports are available.");
        ...
        Logger.getLogger().m891d("Waiting for send/deleteUnsentReports to be called.");
    ...
    public boolean didCrashOnPreviousExecution() {
        ...
        Logger.getLogger().m897v("Found previous crash marker.");
    ...
    public Task<Boolean> checkForUnsentReports() {
        if (!this.checkForUnsentReportsCalled.compareAndSet(false, true)) {
            Logger.getLogger().m899w("checkForUnsentReports should only be called once per execution.");
    ...
    public void submitAllReports(final Task<Settings> task) {
        if (!this.reportingCoordinator.hasReportsToSend()) {
            Logger.getLogger().m897v("No crash reports are available to be sent.");
            this.unsentReportsAvailable.trySetResult(false);
        } else {
            Logger.getLogger().m897v("Crash reports are available to be sent.");
            waitForReportAction().onSuccessTask(this.crashlyticsWorkers.common, new SuccessContinuation<Boolean, Void>() { // from class: com.google.firebase.crashlytics.internal.common.CrashlyticsController.4
                ...
                public Task<Void> then(Boolean bool) throws Exception {
                    if (!bool.booleanValue()) {
                        Logger.getLogger().m897v("Deleting cached crash reports...");
                        CrashlyticsController.deleteFiles(CrashlyticsController.this.listAppExceptionMarkerFiles());
                        CrashlyticsController.this.reportingCoordinator.removeAllReports();
                        CrashlyticsController.this.unsentReportsHandled.trySetResult(null);
                        return Tasks.forResult(null);
                    }
                    Logger.getLogger().m891d("Sending cached crash reports...");
                    ...
                    return task.onSuccessTask(CrashlyticsController.this.crashlyticsWorkers.common, new SuccessContinuation<Settings, Void>() { // from class: com.google.firebase.crashlytics.internal.common.CrashlyticsController.4.1
                        ...
                        public Task<Void> then(Settings settings) throws Exception {
                            ...
                            Logger.getLogger().m899w("Received null app settings at app startup. Cannot send cached reports");
                            return Tasks.forResult(null);
                        }
                    });
                }
    ...
    public void writeNonFatalException(Thread thread, Throwable th) {
        ...
        String currentSessionId = getCurrentSessionId();
        if (currentSessionId == null) {
            Logger.getLogger().m899w("Tried to write a non-fatal exception while no session was open.");
        } else {
    ...
    public void logFatalException(Thread thread, Throwable th) {
        ...
        if (settingsProvider == null) {
            Logger.getLogger().m899w("settingsProvider not set");
        } else {
    ...
    public void setCustomKey(String str, String str2) {
        try {
            ...
            Logger.getLogger().m893e("Attempting to set custom attribute with null key, ignoring.");
        }
    }
    ...
    void openSession(final String str) {
        this.crashlyticsWorkers.common.submit(new Runnable() { // from class: com.google.firebase.crashlytics.internal.common.CrashlyticsController$$ExternalSyntheticLambda1
            ...
            public final void run() {
                CrashlyticsController.this.m901xe0c67ca9(str);
            }
        });
    }

    /* JADX INFO: Access modifiers changed from: package-private */
    /* renamed from: lambda$openSession$1$com-google-firebase-crashlytics-internal-common-CrashlyticsController */
    public /* synthetic */ void m901xe0c67ca9(String str) {
    ...
    public boolean finalizeSessions(SettingsProvider settingsProvider) {
        CrashlyticsWorkers.checkBackgroundThread();
        if (isHandlingException()) {
            Logger.getLogger().m899w("Skipping session finalization because a crash has already occurred.");
            return Boolean.FALSE.booleanValue();
        }
        Logger.getLogger().m897v("Finalizing previously open sessions.");
        try {
            doCloseSessions(true, settingsProvider, true);
            Logger.getLogger().m897v("Closed all previously open sessions.");
            return true;
        } catch (Exception e) {
            Logger.getLogger().m894e("Unable to finalize previously open sessions.", e);
            return false;
        }
    }
    ...
    public void doOpenSession(String str, Boolean bool) {
        ...
        Logger.getLogger().m891d("Opening a new session with ID " + str);
    ...
    private void doCloseSessions(boolean z, SettingsProvider settingsProvider, boolean z2) {
        ...
        if (arrayList.size() <= z) {
            Logger.getLogger().m897v("No open sessions to be closed.");
            return;
        }
        ...
        if (z2 && settingsProvider.getSettingsSync().featureFlagData.collectAnrs) {
            ...
            Logger.getLogger().m897v("ANR feature disabled.");
        }
    ...
    public void saveVersionControlInfo() {
        try {
            ...
            if (versionControlInfo != null) {
                ...
                Logger.getLogger().m895i("Saved version control info");
            }
        } catch (IOException e) {
            Logger.getLogger().m900w("Unable to save version control info", e);
        }
    }
    ...
    String getVersionControlInfo() throws IOException {
        ...
        Logger.getLogger().m891d("Read version control info");
    ...
    private InputStream getResourceAsStream(String str) {
        ...
        if (classLoader == null) {
            Logger.getLogger().m899w("Couldn't get Class Loader");
            return null;
        }
        ...
        Logger.getLogger().m895i("No version control information found");
        return null;
    }
    ...
    private void finalizePreviousNativeSession(String str) {
        Logger.getLogger().m897v("Finalizing native report for session " + str);
        ...
        if (nativeCoreAbsent(str, minidumpFile, applicationExitInto)) {
            Logger.getLogger().m899w("No native core present");
            return;
        }
        ...
        if (!nativeSessionDir.isDirectory()) {
            Logger.getLogger().m899w("Couldn't create directory to store native session files, aborting.");
            return;
        }
        ...
        Logger.getLogger().m891d("CrashlyticsController#finalizePreviousNativeSession");
    ...
    private static boolean nativeCoreAbsent(String str, File file, CrashlyticsReport.ApplicationExitInfo applicationExitInfo) {
        if (file == null || !file.exists()) {
            Logger.getLogger().m899w("No minidump data found for session " + str);
        }
        if (applicationExitInfo == null) {
            Logger.getLogger().m895i("No Tombstones data found for session " + str);
        }
    ...
    public void doWriteAppExceptionMarker(long j) {
        try {
            ...
            Logger.getLogger().m900w("Could not create app exception marker file.", e);
        }
    }
    ...
    public Task<Void> logAnalyticsAppExceptionEvents() {
        ...
        for (File file : listAppExceptionMarkerFiles()) {
            try {
                ...
                Logger.getLogger().m899w("Could not parse app exception timestamp from file " + file.getName());
            }
            file.delete();
        }
    ...
    private Task<Void> logAnalyticsAppExceptionEvent(final long j) {
        if (firebaseCrashExists()) {
            Logger.getLogger().m899w("Skipping logging Crashlytics event to Firebase, FirebaseCrash exists");
            return Tasks.forResult(null);
        }
        Logger.getLogger().m891d("Logging app exception event to Firebase Analytics");
    ...
    private void writeApplicationExitInfoEventIfRelevant(String str) {
        ...
        if (Build.VERSION.SDK_INT >= 30) {
            ...
            if (historicalProcessExitReasons.size() != 0) {
                ...
                Logger.getLogger().m897v("No ApplicationExitInfo available. Session: " + str);
                return;
            }
        }
        Logger.getLogger().m897v("ANR feature enabled, but device is API " + Build.VERSION.SDK_INT);
    }
}
