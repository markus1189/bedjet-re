...
public class FileStore {
    ...
    private void cleanupFileSystemDir(String str) {
        ...
        if (file.exists() && recursiveDelete(file)) {
            Logger.getLogger().m891d("Deleted previous Crashlytics file system: " + file.getPath());
        }
    }
    ...
    private static synchronized File prepareBaseDir(File file) {
        synchronized (FileStore.class) {
            if (file.exists()) {
                ...
                Logger.getLogger().m891d("Unexpected non-directory file: " + file + "; deleting file and creating new directory.");
                file.delete();
            }
            if (!file.mkdirs()) {
                Logger.getLogger().m893e("Could not create Crashlytics-specific directory: " + file);
            }
            return file;
        }
    }
