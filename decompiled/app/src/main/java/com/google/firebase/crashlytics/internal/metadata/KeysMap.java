...
class KeysMap {
    ...
    public synchronized boolean setKey(String str, String str2) {
        ...
        if (this.keys.size() >= this.maxEntries && !this.keys.containsKey(sanitizeKey)) {
            Logger.getLogger().m899w("Ignored entry \"" + str + "\" when adding custom keys. Maximum allowable: " + this.maxEntries);
            return false;
        }
    ...
    public synchronized void setKeys(Map<String, String> map) {
        ...
        if (i > 0) {
            Logger.getLogger().m899w("Ignored " + i + " entries when adding custom keys. Maximum allowable: " + this.maxEntries);
        }
    }
