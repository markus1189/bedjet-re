...
public abstract class LinearTransformation {
    ...
    public static final class LinearTransformationBuilder {
        ...
        /* renamed from: x1 */
        private final double f282x1;

        /* renamed from: y1 */
        private final double f283y1;
        ...
        public LinearTransformation and(double d, double d2) {
            Preconditions.checkArgument(DoubleUtils.isFinite(d) && DoubleUtils.isFinite(d2));
            double d3 = this.f282x1;
            if (d == d3) {
                Preconditions.checkArgument(d2 != this.f283y1);
                return new VerticalLinearTransformation(this.f282x1);
            }
            return withSlope((d2 - this.f283y1) / (d - d3));
        }

        public LinearTransformation withSlope(double d) {
            Preconditions.checkArgument(!Double.isNaN(d));
            if (DoubleUtils.isFinite(d)) {
                return new RegularLinearTransformation(d, this.f283y1 - (this.f282x1 * d));
            }
            return new VerticalLinearTransformation(this.f282x1);
        }
    }
    ...
    public static final class VerticalLinearTransformation extends LinearTransformation {
        ...
        public String toString() {
            return String.format("x = %g", Double.valueOf(this.f284x));
        }

        private LinearTransformation createInverse() {
            return new RegularLinearTransformation(FirebaseRemoteConfig.DEFAULT_VALUE_FOR_DOUBLE, this.f284x, this);
        }
    }
