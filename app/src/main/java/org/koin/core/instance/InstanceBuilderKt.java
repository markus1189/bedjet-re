...
public final class InstanceBuilderKt {
    ...
    public static final <T> T newInstance(Scope scope, KClass<T> kClass, ParametersHolder params) {
        ...
        if (scope.getLogger().getLevel() == Level.DEBUG) {
            scope.getLogger().debug("|- creating new instance - " + KClassExt.getFullName(kClass));
        }
        Constructor<?>[] constructors = JvmClassMapping.getJavaClass((KClass) kClass).getConstructors();
        Intrinsics.checkNotNullExpressionValue(constructors, "getConstructors(...)");
        Constructor constructor = (Constructor) ArraysKt.firstOrNull(constructors);
        if (constructor == null) {
            throw new IllegalStateException(("No constructor found for class '" + KClassExt.getFullName(kClass) + '\'').toString());
        }
        if (scope.getLogger().getLevel() != Level.DEBUG) {
