...
public final class StandardMutableNetwork<N, E> extends StandardNetwork<N, E> implements MutableNetwork<N, E> {
    ...
    public boolean addEdge(N n, N n2, E e) {
        ...
        if (containsEdge(e)) {
            ...
            EndpointPair m828of = EndpointPair.m828of(this, n, n2);
            Preconditions.checkArgument(incidentNodes.equals(m828of), "Edge %s already exists between the following nodes: %s, so it cannot be reused to connect the following nodes: %s.", e, incidentNodes, m828of);
            return false;
        }
    ...
    private NetworkConnections<N, E> newConnections() {
        if (isDirected()) {
            if (allowsParallelEdges()) {
                return DirectedMultiNetworkConnections.m825of();
            }
            return DirectedNetworkConnections.m826of();
        }
        if (allowsParallelEdges()) {
            return UndirectedMultiNetworkConnections.m831of();
        }
        return UndirectedNetworkConnections.m832of();
    }
}
