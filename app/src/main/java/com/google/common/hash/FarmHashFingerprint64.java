...
final class FarmHashFingerprint64 extends AbstractNonStreamingHashFunction {
    ...
    /* renamed from: K0 */
    private static final long f240K0 = -4348849565147123417L;

    /* renamed from: K1 */
    private static final long f241K1 = -5435081209227447693L;

    /* renamed from: K2 */
    private static final long f242K2 = -7286425919675154353L;
    ...
    private static long hashLength0to16(byte[] bArr, int i, int i2) {
        if (i2 >= 8) {
            long j = (i2 * 2) + f242K2;
            long load64 = LittleEndianByteArray.load64(bArr, i) + f242K2;
        ...
        if (i2 >= 4) {
            return hashLength16(i2 + ((LittleEndianByteArray.load32(bArr, i) & 4294967295L) << 3), LittleEndianByteArray.load32(bArr, (i + i2) - 4) & 4294967295L, (i2 * 2) + f242K2);
        }
        if (i2 <= 0) {
            return f242K2;
        }
        return shiftMix((((bArr[i] & 255) + ((bArr[(i2 >> 1) + i] & 255) << 8)) * f242K2) ^ ((i2 + ((bArr[i + (i2 - 1)] & 255) << 2)) * f240K0)) * f242K2;
    }

    private static long hashLength17to32(byte[] bArr, int i, int i2) {
        long j = (i2 * 2) + f242K2;
        long load64 = LittleEndianByteArray.load64(bArr, i) * f241K1;
        long load642 = LittleEndianByteArray.load64(bArr, i + 8);
        int i3 = i + i2;
        long load643 = LittleEndianByteArray.load64(bArr, i3 - 8) * j;
        return hashLength16((LittleEndianByteArray.load64(bArr, i3 - 16) * f242K2) + Long.rotateRight(load64 + load642, 43) + Long.rotateRight(load643, 30), load64 + Long.rotateRight(load642 + f242K2, 18) + load643, j);
    }
    ...
    private static long hashLength33To64(byte[] bArr, int i, int i2) {
        ...
        int i3 = i + i2;
        long load643 = LittleEndianByteArray.load64(bArr, i3 - 8) * j;
        long rotateRight = Long.rotateRight(load64 + load642, 43) + Long.rotateRight(load643, 30) + (LittleEndianByteArray.load64(bArr, i3 - 16) * f242K2);
        long hashLength16 = hashLength16(rotateRight, load643 + Long.rotateRight(load642 + f242K2, 18) + load64, j);
    ...
    private static long hashLength65Plus(byte[] bArr, int i, int i2) {
        long j = 81;
        long j2 = (j * f241K1) + 113;
        long shiftMix = shiftMix((j2 * f242K2) + 113) * f242K2;
        long[] jArr = new long[2];
        long[] jArr2 = new long[2];
        long load64 = (j * f242K2) + LittleEndianByteArray.load64(bArr, i);
        int i3 = i2 - 1;
        int i4 = i + ((i3 / 64) * 64);
        int i5 = i3 & 63;
        int i6 = i4 + i5;
        ...
        int i8 = i;
        while (true) {
            long rotateRight = Long.rotateRight(load64 + j2 + jArr[0] + LittleEndianByteArray.load64(bArr, i8 + 8), 37) * f241K1;
            long rotateRight2 = Long.rotateRight(j2 + jArr[1] + LittleEndianByteArray.load64(bArr, i8 + 48), 42) * f241K1;
            long j3 = rotateRight ^ jArr2[1];
            long load642 = rotateRight2 + jArr[0] + LittleEndianByteArray.load64(bArr, i8 + 40);
            long rotateRight3 = Long.rotateRight(shiftMix + jArr2[0], 33) * f241K1;
            weakHashLength32WithSeeds(bArr, i8, jArr[1] * f241K1, j3 + jArr2[0], jArr);
            weakHashLength32WithSeeds(bArr, i8 + 32, rotateRight3 + jArr2[1], load642 + LittleEndianByteArray.load64(bArr, i8 + 16), jArr2);
            int i9 = i8 + 64;
            if (i9 == i4) {
                long j4 = f241K1 + ((j3 & 255) << 1);
                long j5 = jArr2[0] + i5;
                jArr2[0] = j5;
                long j6 = jArr[0] + j5;
                jArr[0] = j6;
                ...
                return hashLength16(hashLength16(jArr[0], jArr2[0], j4) + (shiftMix(load643) * f240K0) + j7, hashLength16(jArr[1], jArr2[1], j4) + rotateRight6, j4);
            }
            i8 = i9;
            shiftMix = j3;
            j2 = load642;
