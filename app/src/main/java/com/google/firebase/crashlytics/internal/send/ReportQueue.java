...
public final class ReportQueue {
    ...
    public TaskCompletionSource<CrashlyticsReportWithSessionId> enqueueReport(CrashlyticsReportWithSessionId crashlyticsReportWithSessionId, boolean z) {
        synchronized (this.queue) {
            ...
            if (z) {
                ...
                if (isQueueAvailable()) {
                    Logger.getLogger().m891d("Enqueueing report: " + crashlyticsReportWithSessionId.getSessionId());
                    Logger.getLogger().m891d("Queue size: " + this.queue.size());
                    this.singleThreadExecutor.execute(new ReportRunnable(crashlyticsReportWithSessionId, taskCompletionSource));
                    Logger.getLogger().m891d("Closing task for report: " + crashlyticsReportWithSessionId.getSessionId());
                    taskCompletionSource.trySetResult(crashlyticsReportWithSessionId);
                    return taskCompletionSource;
                }
                calcStep();
                Logger.getLogger().m891d("Dropping report due to queue being full: " + crashlyticsReportWithSessionId.getSessionId());
    ...
    public void flushScheduledReportsIfAble() {
        ...
        new Thread(new Runnable() { // from class: com.google.firebase.crashlytics.internal.send.ReportQueue$$ExternalSyntheticLambda0
            ...
            public final void run() {
                ReportQueue.this.m917x23ee29ee(countDownLatch);
            }
        }).start();
    ...
    public /* synthetic */ void m917x23ee29ee(CountDownLatch countDownLatch) {
        try {
    ...
    public void sendReport(final CrashlyticsReportWithSessionId crashlyticsReportWithSessionId, final TaskCompletionSource<CrashlyticsReportWithSessionId> taskCompletionSource) {
        Logger.getLogger().m891d("Sending report through Google DataTransport: " + crashlyticsReportWithSessionId.getSessionId());
        ...
        this.transport.schedule(Event.ofUrgent(crashlyticsReportWithSessionId.getReport()), new TransportScheduleCallback() { // from class: com.google.firebase.crashlytics.internal.send.ReportQueue$$ExternalSyntheticLambda1
            ...
            public final void onSchedule(Exception exc) {
                ReportQueue.this.m918xc033738(taskCompletionSource, z, crashlyticsReportWithSessionId, exc);
            }
        });
    }
    ...
    public /* synthetic */ void m918xc033738(TaskCompletionSource taskCompletionSource, boolean z, CrashlyticsReportWithSessionId crashlyticsReportWithSessionId, Exception exc) {
        if (exc != null) {
    ...
    private final class ReportRunnable implements Runnable {
        ...
        public void run() {
            ...
            Logger.getLogger().m891d("Delay for: " + String.format(Locale.US, "%.2f", Double.valueOf(calcDelay / 1000.0d)) + " s for report: " + this.reportWithSessionId.getSessionId());
